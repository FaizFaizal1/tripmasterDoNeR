...
<style>
    ...
    .day-block { background: #f8f9fa; padding: 20px; margin-bottom: 25px; border-left: 6px solid #ffc72c; }
    .activity-row { display: flex; gap: 10px; margin-top: 10px; }
    input { padding: 8px; }
    .save-btn { width: 100%; padding: 15px; background: #ffc72c; border: none; cursor: pointer; font-weight: bold; }
</style>
</head>
<body>
        ...
        <form action="ActivitiesController" method="POST">
            ...
            <div id="daysContainer">
                ...
                for (String dayTitle : distinctDays) {
                    ...
                    for(Activities a : actList) {
                        if(a.getTitle().equals(dayTitle)) { dayDate = a.getDate(); break; }
                    }
                %>
...
<script>
    // Initialize count based on DB data
    ...
    function addActivity(dayId) {
        ...
        newRow.innerHTML = `
            <input type="time" name="activityTime_`+dayId+`[]">
            <input type="text" name="activityName_`+dayId+`[]">
            <button type="button" onclick="this.parentElement.remove()">X</button>`;
    ...
    function addDay() {
        ...
        // Add similar addDay logic from your create page here...
