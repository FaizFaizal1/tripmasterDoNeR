            ...
            <%@ page import="java.util.List" %>
                <%@ page import="java.util.ArrayList" %>
                    ...
                    <body>
                        ...
                        <% Itinerary itin=(Itinerary) request.getAttribute("itinerary"); List<Activities> actList =
                            (List<Activities>) request.getAttribute("activityList");
                                ...
                                List<String> distinctDays = new ArrayList<>();
                                            ...
                                            <div class="container">
                                                ...
                                                <form action="ActivitiesController" method="POST">
                                                    ...
                                                    <input type="hidden" name="itineraryId"
                                                        value="<%= itin.getItinerary_ID() %>">
                                                    <input type="hidden" id="dayCountInput" name="totalDays"
                                                        value="<%= distinctDays.size() %>">
                                                    <input type="hidden" name="source" value="<%= sourceParam %>">

                                                    <label>Trip Title:</label><br>
                                                    <input type="text" name="itineraryTitle"
                                                        value="<%= itin.getCreate() %>"
                                                        style="width:100%; margin-bottom: 20px;" required>

                                                    <div id="daysContainer">
                                                        <% int dayIndex=0; for (String dayTitle : distinctDays) {
                                                            dayIndex++; java.util.Date dayDate=null; for (Activities a :
                                                            actList) { if (a.getTitle().equals(dayTitle)) {
                                                            dayDate=a.getDate(); break; } } %>
                                                            <div class="day-block" id="day<%= dayIndex %>">
                                                                <label>Day <%= dayIndex %> Details:</label><br>
                                                                <input type="text" name="dayTitle_<%= dayIndex %>"
                                                                    value="<%= dayTitle %>" required>
                                                                <input type="date" name="dayDate_<%= dayIndex %>"
                                                                    value="<%= dayDate %>" required>

                                                                <% for (Activities act : actList) { if
                                                                    (act.getTitle().equals(dayTitle)) { %>
                                                                    <div class="activity-row">
                                                                        <input type="time"
                                                                            name="activityTime_<%= dayIndex %>[]"
                                                                            value="<%= act.getPlan() %>" required>
                                                                        <input type="text"
                                                                            name="activityName_<%= dayIndex %>[]"
                                                                            value="<%= act.getEvent() %>" required>
                                                                        <button type="button"
                                                                    ...
                                                                    <% } } %>
                                                                        <br>
                                                                        <button type="button"
                                                                            onclick="addActivity(<%= dayIndex %>)">+ Add
                                                                            Activity</button>
                                                            </div>
                                                            <% } %>
                                                    </div>
                                        ...
                                        <script>
                                            ...
                                            function addDay() {
                                                ...
                                                newDay.innerHTML = `
            <label>Day ` + dayCount + ` Details:</label><br>
            <input type="text" name="dayTitle_` + dayCount + `" placeholder="Day Title..." required>
